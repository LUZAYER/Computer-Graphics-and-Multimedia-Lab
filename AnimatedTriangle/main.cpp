...
// If R pressed, triangle becomes red permanently
if (forceRed)
{
...
{
    // Holding W makes the triangle white, otherwise it animates between cyan and magenta
    ...
    {
        // Cyan is (0, 1, 1) and magenta is (1, 0, 1), so we can animate between those two colors by using the same value for red and green, and keeping blue at 1.0..
    ...
    // If R is pressed, we set the forceRed flag to true, which will make the triangle red permanently in subsequent frames.
...
{
    //As my name starts with R, and it is used previously, I will use my second name's first letter I.
